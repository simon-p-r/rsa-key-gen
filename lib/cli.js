...
exports.run = (args, cb) => {
    ...
    if (argv instanceof Error) {
        return process.nextTick(() => {
            ...
            cb(new Error(Bossy.usage(definition, argv.message), null));
        });
    }
    ...
    if (argv.help) {
        return process.nextTick(() => {
            ...
            cb(null, Bossy.usage(definition, 'rsa-gen [options]'));
        });
    }
    ...
    if (argv.version || argv.v) {
        return process.nextTick(() => {
            ...
            cb(null, `Package ${Pkg.name} v${Pkg.version}`);
        });
    }
        ...
        if (outDir) {
            RSA.saveKeyPairs(outDir, keyPair, (err) => {
                ...
                return process.nextTick(() => {
                    ...
                    cb(null, `Saved public and private key to directory ${outDir}`);
                });
            });
        }
        else {
