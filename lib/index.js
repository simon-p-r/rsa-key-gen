...
exports.generateKeyPair = function () {
    ...
    if (valid.error) {
        return process.nextTick(() => {
            ...
            callback(internals.humaniseJoi(valid.error), null);
        });
    }

    const bits = valid.value.bits;

    const settings = {
        cipher: valid.value.cipher,
    ...
    Pem.createPrivateKey(bits, settings, (err, privateKey) => {
        ...
        Pem.getPublicKey(privateKey.key, (err, publicKey) => {
            ...
            return process.nextTick(() => {
                ...
                callback(null, { private: privateKey.key, public: publicKey.publicKey });
            });
